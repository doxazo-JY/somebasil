"""PDF + DB(활성 메뉴 66개) 기반 레시피 템플릿 엑셀 v2.

변경점 (v1 → v2):
- 시트1 재료: '단가' 직접 입력 → '구매현황(용량/세트수/총가격) → 단가 수식 자동계산'
- 시트3 레시피: DB의 활성 메뉴 66개 모두 사전 등재 + PDF 매칭되는 사용량 채움
- 시트4 포장세트: 카테고리 영문 키 (coffee/matcha/...) 로
- 적용시작일: 모두 2025-12-15
"""
import json
...
OUT = r"C:\Users\admin\Desktop\지농\coding\somebasil\notes\레시피_템플릿_v2.xlsx"
MENU_DATA = r"C:\Users\admin\Desktop\지농\coding\somebasil\notes\_menu_data.json"
UNK = "❓"
EFF_DATE = "2025-12-15"
...
NEW_MENU_FILL = PatternFill("solid", fgColor="E8F1ED")  # PDF 미수록 메뉴
...
ws = wb.active
ws.title = "README"
ws["A1"] = "썸바실 레시피·원가 입력 양식 (v2 — DB 매칭 + 구매기반 단가)"
ws["A1"].font = Font(name="맑은 고딕", bold=True, size=15, color="1A5C3A")
ws.row_dimensions[1].height = 28

guide = [
    "",
    "이 파일은 PDF 레시피정리(2026.02) + DB의 활성 메뉴 66개를 합쳐 자동 생성된 초안입니다.",
    "",
    "■ 시트 구성",
    ...
    "    → 단가 = 35원/g 자동계산",
    "  바닐라시럽 한 병이 750ml이고 펌프 200회 나오고 30,000원이면:",
    ...
    "  • 시트2 수제재료별 1배치 산출량 + 구성재료 (자몽청, 말차크림, 티라미수베이스, 베이커리류 등)",
    "  • 시트3 PDF 미수록 메뉴(연두색)의 레시피 — 36개",
...
# 구매 컬럼은 일단 ❓로 두고, 단가는 수식 (총가격 / (용량 × 세트수))
for r, (name, unit, memo) in enumerate(purchased, start=2):
    row = r  # actual excel row
    ws.cell(row=row, column=1, value=name)
    ws.cell(row=row, column=2, value=unit)
    ws.cell(row=row, column=3, value="매입")
    ws.cell(row=row, column=4, value=UNK)  # 1세트 용량
    ws.cell(row=row, column=5, value=UNK)  # 세트 수
    ws.cell(row=row, column=6, value=UNK)  # 총가격
    # 단가 수식: =F/(D*E) — 단 D,E,F 모두 채워졌을 때만
    ws.cell(row=row, column=7, value=f"=IFERROR(F{row}/(D{row}*E{row}),\"\")")
    ws.cell(row=row, column=8, value=EFF_DATE)
...
for i, (name, unit, memo) in enumerate(made):
    ...
    ws.cell(row=row, column=7, value="(서브레시피 자동계산)")
...
# PDF → DB 메뉴 매핑 (DB 메뉴명 기준으로 레시피 채우기)
# key: DB의 정확한 name, value: [(재료, 사용량, 비고), ...]
pdf_recipes = {
    "에스프레소": [
        ("원두", 18, "2샷"),
    ],
    "아메리카노": [
        ("원두", 18, "2샷 / HOT·ICE 통일 (물·얼음 무시)"),
    ],
    "카페라떼": [
        ("원두", 18, ""),
...
packs = [
    ...
    # beverage (포장병 — 딸기라떼·바닐라빈밀크)
    ("음료 ICE 포장", "beverage", "ice", "포장병", 1, "❓딸기라떼/바닐라빈밀크 기준"),
    # drip_coffee, dutch_coffee, season — PDF 미수록
