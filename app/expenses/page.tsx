...
export default async function ExpensesPage({ searchParams }: PageProps) {
  ...
  const [summary, expensesByCategory, trend, laborDetail, expenseItems] = await Promise.all([
    getMonthlySummary(year, month),
    getMonthlyExpensesByCategory(year, month),
    getYearlyExpenseTrend(year),
    getMonthlyLaborDetail(year, month),
    ...
    <div className="px-16 py-8 w-full">
      ...
      <div className="flex items-start justify-between mb-6">
        <div>
          ...
          <p className="text-sm text-gray-400 mt-0.5">항목별 지출 분석 및 인건비 현황</p>
        </div>
